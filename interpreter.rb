...
class DefNode
  def eval(context)
    ...
    method = RumMethod.new(params, body)
...
class ClassNode
  def eval(context)
    ...
    rum_class = context[name]
    
    unless rum_class # Class doesn't exist yet
      rum_class = RumClass.new
      # Register the class as a constant in the runtime.
      context[name] = rum_class
    end
    ...
    class_context = Context.new(rum_class, rum_class)
